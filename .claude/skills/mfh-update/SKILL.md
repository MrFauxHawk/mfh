...
- Read `.mfh/design/milestones.md` (next unstarted phase(s), and every currently-⬜ phase across active tracks, including its **Priority** column where present — the latter feeds the `client` profile's planned-work/backlog table and the `personal` profile's top-priority highlight) and `.mfh/design/roadmap.md` (Goals) for the forward-looking half.
